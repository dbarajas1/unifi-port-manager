...
// ── Config ────────────────────────────────────────────────────────────────────
// Env vars take precedence over port-config.json.
// In HA add-on mode all config comes from env vars set by run.sh (no file needed).
// In standalone mode (Mac) port-config.json is used as the source.
...
const controllerUrl = process.env.UNIFI_CONTROLLER || fileConfig.controllerUrl || '';
const site          = process.env.UNIFI_SITE       || fileConfig.site          || 'default';
const username      = process.env.UNIFI_USERNAME   || fileConfig.username      || '';
const deviceName    = process.env.UNIFI_DEVICE     || fileConfig.deviceName    || '';
const ucgFiberName  = process.env.UNIFI_UCG_FIBER  || fileConfig.ucgFiberName  || null;
const apiToken      = process.env.UNIFI_API_TOKEN  || fileConfig.apiToken      || '';
const listenPort    = parseInt(process.env.PORT     || fileConfig.apiPort || 8765, 10);
const listenAddress = process.env.LISTEN_ADDR       || '127.0.0.1';

const missing = [];
if (!controllerUrl)                                             missing.push('controllerUrl / UNIFI_CONTROLLER');
if (!username)                                                  missing.push('username / UNIFI_USERNAME');
if (!deviceName)                                                missing.push('deviceName / UNIFI_DEVICE');
if (!apiToken || apiToken === 'REPLACE_WITH_A_LONG_RANDOM_SECRET') missing.push('apiToken / UNIFI_API_TOKEN');
if (missing.length) {
  console.error('Error: missing required config:', missing.join(', '));
  console.error('Set env vars (HA add-on) or run Setup-Config.ps1 (Mac) to configure.');
  process.exit(1);
}
...
if (!controllerPassword) {
  ...
  console.error('  Mac:  run Setup-Config.ps1 to save it to the Keychain, or set UNIFI_PASSWORD env var.');
  console.error('  HAOS: enter the password in the add-on Configuration tab.');
  process.exit(1);
}
...
// /data/ is the HA add-on persistent volume; fall back to script directory on Mac.
const stateDir = fs.existsSync('/data') ? '/data' : __dirname;
